...
export const handleUploadSubmit = async (formData: FormData) => {
  ...
  if (fscheck.existsSync(`./src/app/(home)/images/${session.user.uuid}`)) {
    console.log("folder already there no");
    await fs.writeFile(
      `./src/app/(home)/images/${session.user.uuid}/${id}.png`,
      buffer
    );
  } else {
    await fs.mkdir(`./src/app/(home)/images/${session.user.uuid}`);
    await fs.writeFile(
      `./src/app/(home)/images/${session.user.uuid}/${id}.png`,
      buffer
    );
